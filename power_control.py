...
def hostsShutdown():
    ...
    for server in config.Servers.Hosts:
        subprocess.Popen(f"ssh {config.Servers.User}@{server}.local -i {config.Servers.SSHKeyPath} -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {config.Servers.ShutdownCommand}", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).wait()
